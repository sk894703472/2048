			...
			if (board[i][j]!=0) {
				for(var k=0;k<j;k++){
					if(board[i][k]==0&&onBlockHorizontal(i,k,j,board)){
						...
						board[i][j]=0;
						continue;
					}else if(board[i][k]==board[i][j]&&onBlockHorizontal(i,k,j,board)&&!hasConflicted[i][k]){
						//move
						showMoveAnimation(i,j,i,k);
						//adds
						board[i][k]+=board[i][j];
			...
			if (board[i][j]!=0) {
				for(var k=3;k>j;k--){
					if(board[i][k]==0&&onBlockHorizontal(i,j,k,board)){
						...
						board[i][j]=0;
						continue;
					}else if(board[i][k]==board[i][j]&&onBlockHorizontal(i,j,k,board)&&!hasConflicted[i][k]){
						//move
						showMoveAnimation(i,j,i,k);
						//adds
						board[i][k]+=board[i][j];
...
function moveUp(){
	...
	for(var j=0;j<4;j++){
		for(var i=1;i<4;i++){
			if (board[i][j]!=0) {
				for(var k=0;k<i;k++){
					if(board[k][j]==0&&onBlockVertical(j,k,i,board)){
						//move
						showMoveAnimation(i,j,k,j);
						board[k][j]=board[i][j];
						board[i][j]=0;
						continue;
					}else if(board[k][j]==board[i][j]&&onBlockHorizontal(j,k,i,board)&&!hasConflicted[k][j]){
						//move
						showMoveAnimation(i,j,k,j);
						//adds
						board[k][j]+=board[i][j];
...
function moveDown(){
	...
	for(var j=0;j<4;j++){
		for(var i=2;i>=0;i--){
			if (board[i][j]!=0) {
				for(var k=3;k>i;k--){
					if(board[k][j]==0&&onBlockVertical(j,i,k,board)){
						//move
						showMoveAnimation(i,j,k,j);
						board[k][j]=board[i][j];
						board[i][j]=0;
						continue;
					}else if(board[k][j]==board[i][j]&&onBlockVertical(j,i,k,board)&&!hasConflicted[k][j]){
						//move
						showMoveAnimation(i,j,k,j);
						//adds
						board[k][j]+=board[i][j];
